...
import com.qx.service.PositionService;
...
public class PdfController {
    ...
    public ResponseEntity<byte[]> PDF(int[] id, Result result) throws Exception {


        System.out.println(id.length+"===============");

        String filename = "E:\\ideaQx\\_0225_OA\\src\\main\\webapp\\Pdf\\" + UUID.randomUUID() + ".pdf";
        ArrayList<Position> list = new ArrayList<>();
        for (int i : id) {

            System.out.println("id:" + i);
            Position byId = positionService.selectPdf(i);
            list.add(byId);
        }
...
//        document.add(new Paragraph("HD content here"));
        ...
        Paragraph paragraph = new Paragraph("PDF职位数据表", fontTitle);//单元格内容
